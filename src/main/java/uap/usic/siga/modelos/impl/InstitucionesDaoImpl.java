...
public class InstitucionesDaoImpl implements InstitucionesDao {
    ...
    public List<InsSedes> listarInsSedesJQPL() {
        String sql = "SELECT isd FROM InsSedes isd "
                + " WHERE isd.idEstado = 'A' "
                + " ORDER BY isd.sede ";
    ...
    public List<InsUbicacionesOrganicas> listarInsUbicacionesOrganicasIdSedeGET(Long idSede) {
        String sql = "SELECT u FROM InsUbicacionesOrganicas u "
                + " LEFT JOIN u.sedes s "
    ...
    public List<String> listarUbicacionesGET(Long id) {
        String sql = "SELECT u.ubicacionOrganica "
                + " FROM InsUbicacionesOrganicas u "
                + " LEFT JOIN u.sedes s "
                + " WHERE s.idSede =:id "
                + " AND s.idEstado = 'A' ";
        Query q = em.createQuery(sql);
    ...
    public List<InsUnidadesFuncionales> listarUnidadesFuncionalesJPQL() {
        String sql = "SELECT unf "
                + " FROM InsUnidadesFuncionales unf "
                + " WHERE unf.idEstado = 'A' "
    ...
    public List<InsDireccionesFuncionales> listarDireccionesFuncionalesPorIdSedeJPQL(Long idSede) {
        String sql = "SELECT dfn FROM InsDireccionesFuncionales dfn "
                + " LEFT JOIN dfn.insSedes sds "
                + " WHERE sds.idSede=:idSede "
                + " AND sds.idEstado = 'A' "
                + " AND dfn.idEstado = 'A' ";
        Query q = em.createQuery(sql);
        q.setParameter("idSede", idSede);
    ...
    public List<InsUnidadesFuncionales> listarUnidadesFuncionalesPorIdDireccionFuncionalJPQL(Long idDireccionFuncional) {
        String sql = " SELECT ufn FROM InsUnidadesFuncionales ufn "
                + " LEFT JOIN ufn.insDireccionesFuncionales dfn "
                + " WHERE dfn.idDireccionFuncional =:idDireccionFuncional "
                + " AND ufn.idEstado = 'A' "
                + " AND dfn.idEstado = 'A' ";
        Query q = em.createQuery(sql);
    ...
    public InsDireccionesFuncionales buscarDireccionFuncionalPorIdDireccionaFuncionalGET(Long idDireccionFuncional) {
        String sql = "SELECT dnf FROM InsDireccionesFuncionales dnf"
                + " WHERE dnf.idDireccionFuncional =:idDireccionFuncional "
                + " AND dnf.idEstado = 'A' ";
        Query q = em.createQuery(sql);
        q.setParameter("idDireccionFuncional", idDireccionFuncional);
    ...
    public InsUnidadesFuncionales buscarInsUnidadesFuncionalesPorIdUnidadFuncionalGET(Long idUnidadFuncional) {
        String sql = "SELECT unf FROM InsUnidadesFuncionales unf"
	...
	public List<Facultades> listarFacultadesPorIdUniversidadJPQL(Long idUniversidad) {
		 String sql = "SELECT fcl FROM Facultades fcl LEFT JOIN fcl.universidades unv "
	                + " WHERE fcl.idEstado = 'A' "
	                + " AND unv.idUniversidad =:idUniversidad"
	                 + " ORDER BY fcl.facultad";
		    Query q = em.createQuery(sql);
	...
	public List<FclCarreras> listarCarrerasPorIdFacultadJPQL(Long idFacultad) {
		 String sql = "SELECT fcr FROM FclCarreras fcr LEFT JOIN fcr.facultades fcl "
	                + " WHERE fcr.idEstado = 'A' "
	                + " AND fcl.idFacultad =:idFacultad"
	                 + " ORDER BY fcr.carrera";
		    Query q = em.createQuery(sql);
	...
	public List<FclEstamentos> listarEstamentosJPQL() {
		 String sql = "SELECT fle FROM FclEstamentos fle "
	                + " WHERE fle.idEstado = 'A' "
	                 + " ORDER BY fle.estamento";
		    Query q = em.createQuery(sql);
 		     return q.getResultList();
     	}
	...
	@Override
	public List<InsDireccionesFuncionales> listarDireccionesFuncionalesJPQL() {
		  String sql = "SELECT drf FROM InsDireccionesFuncionales drf "
		  	     + "  WHERE drf.idEstado = 'A' "
	              + "  ORDER BY drf.direccionFuncional";
	        Query q = em.createQuery(sql);
	        return q.getResultList();
	}
