...
public class SicoesDaoImpl implements SicoesDao {
	...
	public List<ScsContrataciones> listarContratacionesPorGestionIdScsProyectosJQPL(Long idScsProyecto,Integer gestion) {
	    String sql = "SELECT scs "
                + " FROM ScsContrataciones scs LEFT JOIN scs.scsProyectos scp"
                + " WHERE scs.gestion =:gestion "
                + " AND scp.idScsProyecto =:idScsProyecto"
                + " AND scs.idEstado = 'A' "
                + " AND scp.idEstado = 'A' ";
        Query q = em.createQuery(sql);
        q.setParameter("gestion", gestion);
	...
	public List<ScsContrataciones> listarContratacionesPorGestionIdDireccionesFuncionalesJPQL(Long idDireccionFuncional,	Integer gestion) {
		 String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsProyectos scp "
		 			+ " LEFT JOIN scp.insUnidadesFuncionales inu LEFT JOIN inu.insDireccionesFuncionales drf "
	                + " WHERE scs.gestion =:gestion "
	                + " AND drf.idDireccionFuncional =:idDireccionFuncional "
	                + " AND scs.idEstado = 'A' "
	                + " AND scp.idEstado = 'A'  "
	                + " AND inu.idEstado = 'A' "
	                + " AND drf.idEstado = 'A' ";
	 ...
	 @Override
		public List<ScsContrataciones> listarContratacionesPorGestionIdScsContratadoJQPL(Long idPersona,Integer gestion) {
		 String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsPrsContratados scp "
		 			+ " LEFT JOIN scp.personas prs "
	                + " WHERE scs.gestion =:gestion "
	                + " AND prs.idPersona =:idPersona "
	                + " AND scs.idEstado = 'A' "
	                + " AND scp.idEstado = 'A'  "
	                + " AND prs.idEstado = 'A' ";
	        Query q = em.createQuery(sql);
	...
	public List<ScsContrataciones> listarContratacionesPorIdDireccionesFuncionalesJPQL(Long idDireccionFuncional) {
		 String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsProyectos scp "
		 			+ " LEFT JOIN scp.insUnidadesFuncionales inu LEFT JOIN inu.insDireccionesFuncionales drf "
	                + " WHERE drf.idDireccionFuncional =:idDireccionFuncional "
	                + " AND scs.idEstado = 'A' "
	                + " AND scp.idEstado = 'A'  "
	                + " AND inu.idEstado = 'A' "
	                + " AND drf.idEstado = 'A' ";
	...
	@Override
	public List<ScsContrataciones> listarContratacionesPorGestionIdScsUnidadesFuncionalesJQPL(Long idUnidadFuncional,
			Integer gestion) {
		 String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsProyectos scp "
		 			+ " LEFT JOIN scp.insUnidadesFuncionales inu  "
	                + " WHERE scs.gestion =:gestion "
	                + " AND inu.idUnidadFuncional =:idUnidadFuncional "
	                + " AND scs.idEstado = 'A' "
	                + " AND scp.idEstado = 'A'  "
	                + " AND inu.idEstado = 'A' ";
	        Query q = em.createQuery(sql);
	...
	public List<ScsContrataciones> listarContratacionesPorIdUnidadesFuncionalesJPQL(Long idUnidadFuncional) {
		String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsProyectos scp "
	 			+ " LEFT JOIN scp.insUnidadesFuncionales inu"
                + " WHERE inu.idUnidadFuncional =:idUnidadFuncional "
                + " AND scs.idEstado = 'A' "
                + " AND scp.idEstado = 'A'  "
                + " AND inu.idEstado = 'A' ";
        Query q = em.createQuery(sql);
	...
	public List<ScsContrataciones> listarContratacionesPorIdModalidadJPQL(Long idScsModalidad) {
		String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsModalidades scm "
                + " WHERE scm.idScsModalidad =:idScsModalidad "
                + " AND scs.idEstado = 'A' "
                + " AND scm.idEstado = 'A'  ";
        Query q = em.createQuery(sql);
	...
	public List<ScsContrataciones> listarContratacionesPorGestionModalidadJPQL(Integer gestion) {
		String sql = "SELECT scs FROM ScsContrataciones scs LEFT JOIN scs.scsProyectos scm"
                + " WHERE scs.gestion =:gestion "
                + " AND scs.idEstado = 'A' "
                + " AND scm.idEstado = 'A'  ";
        Query q = em.createQuery(sql);
