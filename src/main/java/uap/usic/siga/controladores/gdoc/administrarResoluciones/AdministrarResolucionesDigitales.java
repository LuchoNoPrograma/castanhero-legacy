...
public class AdministrarResolucionesDigitales {
    ...
    private PdfService pdfService;
	  ...
	  @PostMapping(value = "/generarResolucion")
	    public void generarResolusiones(@RequestParam("idGdocResolucionDigital") Long id, Model model, HttpSession session)  {
		  String htmlContent = "http://localhost:8080/Castanhero/resolucionDigital/generarPdfResol/1";
		  String  templ = "";
	       String  name = "C:/gDoc/holaaa.pdf";
		  pdfService.exportPdfFile(templ, htmlContent, name);
		  //getDesplegarListasComunes(model, session);
	  ...
	  @GetMapping(value = "/generarPdfResol/{id}")
	    public String generarPdfResolusiones(@PathVariable("id") Long idGdocResolucionDigital, Model model, HttpSession session)  {
		   getDesplegarListasComunes(model, session);
		   model.addAttribute("bResolucion", gServicios.buscarGdocResolucionesDigitalesPorIdGdocResolucionesDigitales(idGdocResolucionDigital));
	        model.addAttribute("operation", "reg");
	        return "imprimirResolucion";
